...
select employee_id,job_id,year(hire_date) as hire_year from employee where year(hire_date) between 1990 and 1999;
-- Question 7
...
select employee_id,email,position('A',email)as letter_position from employee;
-- Question 8
...
select first_name,email,substr(phone_number,0,length(phone_number)-length(split_part(phone_number,'.',4))-1) as phone_column,split_part(phone_number,'.',4) as extension_column from employee;
-- Question 13
-- Write a SQL query to find the employee with second and third maximum
-- salary with and without using top/limit keyword
select salary from (select  row_number() over(ORDER BY salary desc) as rownum,salary from (select distinct salary from employee)) where rownum between 2 and 3;
